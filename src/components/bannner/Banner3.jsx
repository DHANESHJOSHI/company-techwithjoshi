...
const DEFAULT_SLIDES = [
  {
    id: "slide-1",
    heading: "Engineering Next-Gen AI & Enterprise Software.",
    description: "We build scalable cloud-native architectures, intelligent AI workflows, and high-performance web applications that drive real enterprise growth.",
...
function Banner3() {
    ...
    const { data: cmsHero } = useCMS("hero");
    const slides = (cmsHero && cmsHero.slides && cmsHero.slides.length > 0) ? cmsHero.slides : DEFAULT_SLIDES;
    ...
    <div className="banner-area3">
        ...
        <Swiper {...slider} className="swiper banner3-slider">
            {slides.map((slide, idx) => (
              <SwiperSlide key={slide.id || idx} className="swiper-slide">
                <div className="banner-wrapper">
                  <div className="banner-content">
                    <h1>{slide.heading}</h1>
                    <p>{slide.description}</p>
                    <div className="banner-btn-group">
                      <Link legacyBehavior href={slide.primaryBtnLink || "/contact"}>
                        <a className="primary-btn3">{slide.primaryBtnText || "Free Consultancy"}</a>
                      </Link>
                      <Link legacyBehavior href={slide.secondaryBtnLink || "/about"}>
                        <a className="primary-btn4">
                          ...
                          {slide.secondaryBtnText || "Explore More"}
                        </a>
                      </Link>
                    </div>
                  </div>
                  <div className="banner-image magnetic-item">
                    <img className="img-fluid" src={slide.image || "assets/img/home-3/home3-banner-img.png"} alt={slide.badge || "TechWithJoshi Engineering"} />
                  </div>
                  <div className="banner-video-area">
                    <div className="video-area">
                      ...
                      <img className="video-img" src="assets/img/home-3/home3-banner-video.png" alt="TechWithJoshi Private Limited" />
                    </div>
                  </div>
                </div>
              </SwiperSlide>
        ...
        <React.Fragment>
			<ModalVideo style={{ overlay: { zIndex: 9999 } }} channel='youtube' autoplay isOpen={isOpen} videoId="w8EMXtbzMSk" onClose={() => setOpen(false)} />
		</React.Fragment>
    </div>
